...
const Chart = lazy(() =>
    import('react-google-charts').then((module) => ({ default: module.Chart }))
)
...
function ReportedPerFarm() {
    ...
    console.log('per farm reports: ', reports)
...
function ReportedDurianDiseaseTypes() {
    ...
    const seriesColors: { [key: string]: string } = {
        'Phytophthora palmivora Fruit Rot (Late Stage)': '#ff7800', // orange
    ...
    if (reports.length > 0) {
        reports.forEach((item) => {
            ...
            const formattedDdt =
                ddt.toLowerCase() === 'durian blight'
                    ? 'Phytophthora palmivora Fruit Rot (Late Stage)'
                    : ddt.toLowerCase() === 'durian spot'
...
function DatasetClasses() {
    ...
    if (datasetclasses) {
        datasetclasses.forEach((item) => {
            const formattedDdt =
                item.name.toLowerCase() === 'durian blight'
                    ? 'Phytophthora palmivora Fruit Rot (Late Stage)'
                    : item.name.toLowerCase() === 'durian spot'
                    ? 'Phytophthora palmivora Fruit Rot (Early Stage)'
                    : item.name
            ...
            {datasets_query.isLoading ? (
                ...
                <>
                    {datasetclasses.length === 0 ? (
                        <div className="h-[70%] w-full flex items-center justify-center">
                            <div className="text-red-800 font-bold text-xl text-red-800 text-center">
